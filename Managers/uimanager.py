...
class UIManager():
    def __init__(self):
        ...
        self.keys = list(self.menu.keys())
        self.menu_index = 0
        self.selected_menu = self.keys[self.menu_index]
    ...
    def get_function(self, mouse_pos):
        if self.menu_vis_button.base_rect.collidepoint(mouse_pos):
            if self.show_menu:
                self.menu_vis_button.base_rect.x = constants.WIDTH - \
                    self.menu_vis_button.base_rect.width
                self.show_menu = False

            else:
                self.menu_vis_button.base_rect.x = self.menu[self.selected_menu].rect.x - \
                    self.menu_vis_button.base_rect.width
                self.show_menu = True
    ...
    def cycle_menu(self, direction):
        if direction == "left" and self.menu_index > 0:
            self.menu_index -= 1

        elif direction == "right" and self.menu_index < len(self.keys) - 1:
            self.menu_index += 1
